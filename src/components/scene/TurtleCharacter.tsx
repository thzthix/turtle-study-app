...
import { hasApprovedSceneAsset, sceneAssetSlots, type TurtleStateAssetKey } from '../../lib/sceneAssetSlots';
...
export function TurtleCharacter({ mouthPath, eyePaths, status, turtleStyle }: TurtleCharacterProps) {
  const turtleStateKey = getTurtleStateAssetKey(status);
  const approvedAssetPath = turtleStateKey === null ? null : sceneAssetSlots.turtleStates[turtleStateKey].approvedAssetPath;

  return (
    <div className={`turtle turtle-${status}`} style={turtleStyle}>
      <span className="turtle-shadow" aria-hidden="true" />
      {hasApprovedSceneAsset(approvedAssetPath) ? (
        <img className="turtle-asset-image" src={approvedAssetPath} alt="" aria-hidden="true" />
      ) : (
...
function getTurtleStateAssetKey(status: SessionStatus): TurtleStateAssetKey | null {
  switch (status) {
    case 'walking':
    case 'care-needed':
      return 'walking';
